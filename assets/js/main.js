...
// --- CORE: GOTO STEP (bisa dipanggil dari mana saja) ---
function goToStep(targetStep) {
    ...
    // Terapkan filter paket ke step yang baru dibuka
...
// Tetap kompatibel dengan tombol prev/next yang pakai changeStep()
...
// --- UPDATE UI ---
function updateUI() {
    // Progress bar lama dihapus, diganti updateStepIndicators()
...
// --- PACKAGE FILTER (dipisah dari toggle supaya bisa dipanggil ulang saat ganti step) ---
function applyPackageFilter(pkg) {
    ...
    // Step 3: diagram wiring & baris tabel plus
...
// --- TOGGLE PACKAGE (dipanggil dari tombol di HTML) ---
...
// ===================== NAVIGATION =====================
// HAPUS semua deklarasi menuBtn, mobileMenu, dll. yang ada di luar DOMContentLoaded
// Ganti dengan satu blok ini saja:
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (desktopDataBtn) {
        desktopDataBtn.addEventListener('click', (e) => {
            ...
            dataArrow.style.transform = isOpen ? 'rotate(0deg)' : 'rotate(180deg)';
        });
    }
...
(function () {
    if (localStorage.getItem('sk-theme') === 'dark') {
        // Tunda sedikit agar elemen tombol sudah ada
...
function submitHelpForm() {
    ...
    if (userAnswer !== captchaAnswer) {
        ...
        status.className = 'material-symbols-outlined text-lg text-error';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ===================== DROPDOWN DATA (desktop) =====================
    const desktopBtn      = document.getElementById('desktop-data-btn');
    const desktopDropdown = document.getElementById('desktop-dropdown');
    const dataArrow       = document.getElementById('data-arrow');
    ...
    document.addEventListener('click', () => {
        desktopDropdown.classList.add('hidden');
        dataArrow.style.transform = 'rotate(0deg)';
    });
...
function switchSubTab(tab) {
    ...
    if (currentPage === 'data.html') {
        // Sudah di data.html, langsung switch tab
        ...
        // Di halaman lain, redirect ke data.html dengan parameter tab
